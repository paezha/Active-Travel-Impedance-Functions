...
heatmap_plot <- ggplot(heatmap_melted, aes(x = Destination, y = Origin, fill = value)) +
  geom_tile(color = "white") +
  scale_fill_gradientn(colors = RColorBrewer::brewer.pal(9, "Blues"), 
                       na.value = "lightgray", 
                       limits = c(0, max_percentage), 
                       breaks = c(0, max_percentage), 
                       labels = c("0%", paste0(round(max_percentage, 2), "%"))) +
...
trip_counts <- walking_2015 %>%
  ...
  summarize(count_each_trip = n(), 
            weighted_count = sum(WGHT_EPI)) # Count trips and sum weights for each origin-destination pair

# Calculate the total number of trips
sum_count_trip <- sum(trip_counts$count_each_trip)

# Calculate the total weighted count for normalization
total_weighted_count <- sum(trip_counts$weighted_count)

# Calculate percentages based on the provided formula
trip_counts <- trip_counts %>%
  mutate(percentage = ((count_each_trip * weighted_count) / (sum_count_trip * total_weighted_count)) * 100)
...
```
```{r}
total_sum <- sum(walking_2015$WGHT_EPI, na.rm = TRUE)
print(total_sum )
...
# Calculate the weighted count of trips for each origin-destination pair
trip_counts <- walking_2015 %>%
  group_by(orig, dest) %>%
  summarize(weighted_count = sum(WGHT_EPI)) # Sum the weights for each origin-destination pair

# Calculate the total weighted count for normalization
total_weighted_count <- sum(trip_counts$weighted_count)

# Convert weighted counts to percentages and round the values
trip_counts <- trip_counts %>%
  mutate(percentage = round((weighted_count / total_weighted_count) * 100, 2)) # Round to 2 decimal places

# Reshape data for cross-tabulation
cross_tab_matrix <- reshape2::dcast(trip_counts, orig ~ dest, value.var = "percentage")

# Calculate row totals
cross_tab_matrix$Total <- rowSums(cross_tab_matrix[,-1], na.rm = TRUE) # Exclude the first column (orig) for row sums

# Calculate column totals and append to the table
total_row <- colSums(cross_tab_matrix[, -1], na.rm = TRUE) # Exclude the first column (orig) for column sums
total_row <- c("Total", total_row) # Add a label for the total row
cross_tab_matrix <- rbind(cross_tab_matrix, total_row)

# View the cross-tabulation table with totals
...
# Round percentages to 2 decimal places
trip_counts <- trip_counts %>%
  mutate(percentage = round(percentage, 5))

# Create the crosstab data frame
crosstab <- dcast(trip_counts, orig ~ dest, value.var = "percentage")

# View the crosstab table
crosstab

# Optionally, create a nicer looking table using knitr and kableExtra
kable(crosstab, "html") %>%
...
# Using kable and kableExtra for a well-formatted table
kable(cross_tab_matrix, "html", booktabs = TRUE) %>%
  kable_styling(full_width = FALSE, position = "center") %>%
  column_spec(1, bold = TRUE, border_right = TRUE) %>%
  row_spec(nrow(cross_tab_matrix), bold = TRUE, color = "white", background = "#56B4E9")
...
# Calculate the weighted number of trips
trip_counts <- walking_2010 %>%
  group_by(orig, dest) %>%
  summarize(count = n(), # Count the number of trips for each origin-destination pair
            average_weight = mean(WGHT_EPI)) %>% # Calculate the average weight for the pair
  mutate(weighted_count = count * average_weight) # Multiply the count by the average weight
...
# Create heatmap
heatmap_plot_2010_weighted <- ggplot(trip_counts, aes(x = orig, y = dest, fill = percentage)) +
  geom_tile(color = "white") +
  scale_fill_gradientn(colors = brewer.pal(9, "Blues")) +
  labs(x = "Origin", y = "Destination", fill = "Percentage") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) # Rotate x-axis labels for better readability

# Display the heatmap
print(heatmap_plot_2010_weighted)
```
...
walking_1986_f <- walking_1986_f %>%
  mutate(f_t86 = coalesce(f_1, f_2, f_3))


walking_data <- bind_rows(walking_2015_f, walking_2010_f, walking_2005_f,walking_1998_f, walking_1992_f,walking_1986_f )
...
cycling_data <- bind_rows(cycling_2015_f, cycling_2010_f, cycling_2005_f,cycling_1998_f, cycling_1992_f )
```
...
# Assuming walking_data is already loaded and has 'YEAR', 'DURATION', 'dest', and 'f_t' columns

# Define the font family to use (this font must be installed on your system)
font_family <- "Helvetica"

# Plot the data
home_plot <- ggplot(walking_data, aes(x = DURATION, y = f_t, group = dest)) +
  # Draw all non-'home' destination lines in grey
  geom_line(data = walking_data %>% filter(dest != "home"), 
            aes(group = dest), color = "grey40", size = 0.5, alpha = 0.5) +
  # Draw 'home' destination lines in cyan and larger size
  geom_line(data = walking_data %>% filter(dest == "home"), 
            aes(group = dest), color = "cyan3", size = 1.2) +
  ...
  # Facet by 'YEAR'
  facet_wrap(~YEAR) +
  # Use a minimal theme with white background
  theme_minimal(base_size = 14) +
  theme(
    plot.title = element_text(hjust = 0.5, size = 16, color = "grey40", family ="Times New Roman"),
    axis.title = element_text(color = "grey40", family = "Times New Roman"),
    axis.text = element_text(color = "grey40", family = "Times New Roman"),
    ...
    strip.background = element_rect(fill = "white", colour = NA)  # White background for facet labels
  ) +
  theme(aspect.ratio = 1/2)  # Set the aspect ratio

# Display the plot
print(home_plot)

# Save the plot if desired
ggsave("home_plot.jpg", plot = home_plot, width = 8, height = 6, dpi = 300)
```
...
# Plot the data
work_plot <- ggplot(walking_data, aes(x = DURATION, y = f_t, group = dest)) +
  # Draw all non-'home' destination lines in grey
  geom_line(data = walking_data %>% filter(dest != "work or school"), 
            aes(group = dest), color = "grey40", size = 0.5, alpha = 0.5) +
  # Draw 'home' destination lines in cyan and larger size
  geom_line(data = walking_data %>% filter(dest == "work or school"), 
            aes(group = dest), color = "deeppink2", size = 1.2) +
  # Set the title and labels
...
# Save the plot if desired
ggsave("work_plot.jpg", plot = work_plot, width = 8, height = 6, dpi = 300)
```
...
# Plot the data
others_home_plot <- ggplot(walking_data, aes(x = DURATION, y = f_t, group = dest)) +
  # Draw all non-'home' destination lines in grey
  geom_line(data = walking_data %>% filter(dest != "other's home"), 
            aes(group = dest), color = "grey40", size = 0.5, alpha = 0.5) +
  # Draw 'home' destination lines in cyan and larger size
  geom_line(data = walking_data %>% filter(dest == "other's home"), 
            aes(group = dest), color = "darkorchid3", size = 1.2) +
  # Set the title and labels
...
# Display the plot
print(others_home_plot)

# Save the plot if desired
ggsave("others_home_plot.jpg", plot = others_home_plot, width = 8, height = 6, dpi = 300)
```
...
# Plot the data
Restaurant_plot <- ggplot(walking_data, aes(x = DURATION, y = f_t, group = dest)) +
  # Draw all non-'home' destination lines in grey
  geom_line(data = walking_data %>% filter(dest != "Restaurant, bar or club"), 
            aes(group = dest), color = "grey40", size = 0.5, alpha = 0.5) +
  # Draw 'home' destination lines in cyan and larger size
  geom_line(data = walking_data %>% filter(dest == "Restaurant, bar or club"), 
            aes(group = dest), color = "yellow2", size = 1.2) +
  # Set the title and labels
  ...
  theme_minimal(base_size = 14) +
  theme(
    plot.title = element_text(hjust = 0.5, size = 14, color = "grey40", family ="Times New Roman"),
    axis.title = element_text(color = "grey40", family = "Times New Roman"),
    axis.text = element_text(color = "grey40", family = "Times New Roman"),
    strip.text = element_text(color = "grey40", family = "Times New Roman"),
    legend.position = "none",
...
# Display the plot
print(Restaurant_plot)

# Save the plot if desired
ggsave("Restaurant_plot.jpg", plot = Restaurant_plot, width = 8, height = 6, dpi = 300)
```
...
# Plot the data
Grocery_plot <- ggplot(walking_data, aes(x = DURATION, y = f_t, group = dest)) +
  # Draw all non-'home' destination lines in grey
  geom_line(data = walking_data %>% filter(dest != "Grocery store, other stores or mall"), 
            aes(group = dest), color = "grey40", size = 0.5, alpha = 0.5) +
  # Draw 'home' destination lines in cyan and larger size
  geom_line(data = walking_data %>% filter(dest == "Grocery store, other stores or mall"), 
            aes(group = dest), color = "red3", size = 1.2) +
  # Set the title and labels
...
# Save the plot if desired
ggsave("Grocery_plot.jpg", plot = Grocery_plot, width = 8, height = 6, dpi = 300)
```
...
# Plot the data
Library_plot <- ggplot(walking_data, aes(x = DURATION, y = f_t, group = dest)) +
  # Draw all non-'home' destination lines in grey
  geom_line(data = walking_data %>% filter(dest != "Library, museum or theatre"), 
            aes(group = dest), color = "grey40", size = 0.5, alpha = 0.5) +
  # Draw 'home' destination lines in cyan and larger size
  geom_line(data = walking_data %>% filter(dest == "Library, museum or theatre"), 
            aes(group = dest), color = "olivedrab3", size = 1.2) +
  # Set the title and labels
...
# Save the plot if desired
ggsave("Library_plot.jpg", plot = Library_plot, width = 8, height = 6, dpi = 300)
```
...
# Plot the data
outdoors_plot <- ggplot(walking_data, aes(x = DURATION, y = f_t, group = dest)) +
  # Draw all non-'home' destination lines in grey
  geom_line(data = walking_data %>% filter(dest != "outdoors"), 
            aes(group = dest), color = "grey40", size = 0.5, alpha = 0.5) +
  # Draw 'home' destination lines in cyan and larger size
  geom_line(data = walking_data %>% filter(dest == "outdoors"), 
            aes(group = dest), color = "royalblue2", size = 1.2) +
  # Set the title and labels
...
# Save the plot if desired
ggsave("outdoors_plot.jpg", plot = outdoors_plot, width = 8, height = 6, dpi = 300)
```
...
# Plot the data
worship_plot <- ggplot(walking_data, aes(x = DURATION, y = f_t, group = dest)) +
  # Draw all non-'home' destination lines in grey
  geom_line(data = walking_data %>% filter(dest != "Place of worship"), 
            aes(group = dest), color = "grey40", size = 0.5, alpha = 0.5) +
  # Draw 'home' destination lines in cyan and larger size
  geom_line(data = walking_data %>% filter(dest == "Place of worship"), 
            aes(group = dest), color = "darkorange1", size = 1.2) +
  # Set the title and labels
...
# Save the plot if desired
ggsave("worship_plot.jpg", plot = worship_plot, width = 8, height = 6, dpi = 300)
```
...
# Plot the data
home_c_plot <- ggplot(cycling_data, aes(x = DURATION, y = f_t, group = dest)) +
  # Draw all non-'home' destination lines in grey
  geom_line(data = cycling_data %>% filter(dest != "home"), 
            aes(group = dest), color = "grey40", size = 0.5, alpha = 0.5) +
  # Draw 'home' destination lines in cyan and larger size
  geom_line(data = cycling_data %>% filter(dest == "home"), 
            aes(group = dest), color = "cyan3", size = 1.2) +
...
print(home_c_plot)
# Save the plot if desired
ggsave("home_c_plot.jpg", plot = home_c_plot, width = 8, height = 6, dpi = 300)
```
...
# Assuming cycling_data is already loaded and has 'YEAR', 'DURATION', 'dest', and 'f_t' columns

# Define the font family to use (this font must be installed on your system)
font_family <- "Helvetica"

# Plot the data
work_c_plot <- ggplot(cycling_data, aes(x = DURATION, y = f_t, group = dest)) +
  # Draw all non-'home' destination lines in grey
  geom_line(data = cycling_data %>% filter(dest != "work or school"), 
            aes(group = dest), color = "grey40", size = 0.5, alpha = 0.5) +
  # Draw 'home' destination lines in cyan and larger size
  geom_line(data = cycling_data %>% filter(dest == "work or school"), 
            aes(group = dest), color = "deeppink2", size = 1.2) +
...
# Save the plot if desired
ggsave("work_c_plot.jpg", plot = work_c_plot, width = 8, height = 6, dpi = 300)
...
# Plot the data
others_home_c_plot <- ggplot(cycling_data, aes(x = DURATION, y = f_t, group = dest)) +
  # Draw all non-'home' destination lines in grey
  geom_line(data = cycling_data %>% filter(dest != "other's home"), 
            aes(group = dest), color = "grey40", size = 0.5, alpha = 0.5) +
  # Draw 'home' destination lines in cyan and larger size
  geom_line(data = cycling_data %>% filter(dest == "other's home"), 
            aes(group = dest), color = "darkorchid3", size = 1.2) +
...
# Display the plot
print(others_home_c_plot)

# Save the plot if desired
ggsave("others_home_c_plot.jpg", plot = others_home_c_plot, width = 8, height = 6, dpi = 300)
```
...
# Plot the data
Restaurant_c_plot <- ggplot(cycling_data, aes(x = DURATION, y = f_t, group = dest)) +
  # Draw all non-'home' destination lines in grey
  geom_line(data = cycling_data %>% filter(dest != "Restaurant, bar or club"), 
            aes(group = dest), color = "grey40", size = 0.5, alpha = 0.5) +
  # Draw 'home' destination lines in cyan and larger size
  geom_line(data = cycling_data %>% filter(dest == "Restaurant, bar or club"), 
            aes(group = dest), color = "yellow2", size = 1.2) +
...
# Display the plot
print(Restaurant_c_plot)

# Save the plot if desired
ggsave("Restaurant_c_plot.jpg", plot = Restaurant_c_plot, width = 8, height = 6, dpi = 300)
```
...
# Plot the data
Grocery_c_plot <- ggplot(cycling_data, aes(x = DURATION, y = f_t, group = dest)) +
  # Draw all non-'home' destination lines in grey
  geom_line(data = cycling_data %>% filter(dest != "Grocery store, other stores or mall"), 
            aes(group = dest), color = "grey40", size = 0.5, alpha = 0.5) +
  # Draw 'home' destination lines in cyan and larger size
  geom_line(data = cycling_data %>% filter(dest == "Grocery store, other stores or mall"), 
            aes(group = dest), color = "red3", size = 1.2) +
...
# Display the plot
print(Grocery_c_plot)
# Save the plot if desired
ggsave("Grocery_c_plot.jpg", plot = Grocery_c_plot, width = 8, height = 6, dpi = 300)
```
...
# Plot the data
outdoors_c_plot <- ggplot(cycling_data, aes(x = DURATION, y = f_t, group = dest)) +
  # Draw all non-'home' destination lines in grey
  geom_line(data = cycling_data %>% filter(dest != "outdoors"), 
            aes(group = dest), color = "grey40", size = 0.5, alpha = 0.5) +
  # Draw 'home' destination lines in cyan and larger size
  geom_line(data = cycling_data %>% filter(dest == "outdoors"), 
            aes(group = dest), color = "royalblue2", size = 1.2) +
...
# Display the plot
print(outdoors_c_plot)

# Save the plot if desired
ggsave("outdoors_c_plot.jpg", plot = outdoors_c_plot, width = 8, height = 6, dpi = 300)
...
walking_data_2015 <- walking_data %>%
  filter(YEAR == 2015)
...
walking_plot_2015 <- ggplot(walking_data_2015, aes(x = DURATION, y = f_t, group = dest, color = dest)) +
  geom_line(size = 1.2) +
  ...
  theme(
    ...
    axis.title = element_text(color = "grey40", family = "Times New Roman"),
    axis.text = element_text(color = "grey40", family = "Times New Roman"),
    legend.text = element_text(size = 10, color = "grey40", family = "Times New Roman"),  # Set legend text properties
    legend.title = element_text(size = 10, color = "grey40", family = "Times New Roman"), # Set legend title properties
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    panel.background = element_rect(fill = "white", colour = NA)  # White background for panels
  )
...
# Display the plot
print(walking_plot_2015)

# Save the plot if desired
ggsave("walking_plot_2015.jpg", plot = walking_plot_2015, width = 8, height = 6, dpi = 300)
...
walking_data_2010 <- walking_data %>%
  filter(YEAR == 2010)
...
# Plot the data for the year 2015
walking_plot_2010 <- ggplot(walking_data_2010, aes(x = DURATION, y = f_t, group = dest, color = dest)) +
  geom_line(size = 1.2) +
  scale_color_manual(values = dest_colors) +
  labs(
...
# Display the plot
print(walking_plot_2010)

# Save the plot if desired
ggsave("walking_plot_2010.jpg", plot = walking_plot_2010, width = 8, height = 6, dpi = 300)
...
walking_data_2005 <- walking_data %>%
  filter(YEAR == 2005)
...
# Plot the data for the year 2015
walking_plot_2005 <- ggplot(walking_data_2005, aes(x = DURATION, y = f_t, group = dest, color = dest)) +
  geom_line(size = 1.2) +
  scale_color_manual(values = dest_colors) +
  labs(
...
# Display the plot
print(walking_plot_2005)

# Save the plot if desired
ggsave("walking_plot_2005.jpg", plot = walking_plot_2005, width = 8, height = 6, dpi = 300)
...
walking_data_1998 <- walking_data %>%
  filter(YEAR == 1998)
...
# Plot the data for the year 2015
walking_plot_1998 <- ggplot(walking_data_1998, aes(x = DURATION, y = f_t, group = dest, color = dest)) +
  geom_line(size = 1.2) +
  scale_color_manual(values = dest_colors) +
  labs(
...
# Display the plot
print(walking_plot_1998)

# Save the plot if desired
ggsave("walking_plot_1998.jpg", plot = walking_plot_1998, width = 8, height = 6, dpi = 300)
...
# Filter the walking_data for the year 1998 only
walking_data_1992 <- walking_data %>%
  filter(YEAR == 1992)
...
walking_plot_1992 <- ggplot(walking_data_1992, aes(x = DURATION, y = f_t, group = dest, color = dest)) +
  geom_line(size = 1.2) +
  scale_color_manual(values = dest_colors) +
  labs(
...
# Display the plot
print(walking_plot_1992)

# Save the plot if desired
ggsave("walking_plot_1992.jpg", plot = walking_plot_1992, width = 8, height = 6, dpi = 300)
...
# Filter the walking_data for the year 1998 only
walking_data_1986 <- walking_data %>%
  filter(YEAR == 1986)
...
walking_plot_1986 <- ggplot(walking_data_1986, aes(x = DURATION, y = f_t, group = dest, color = dest)) +
  geom_line(size = 1.2) +
  scale_color_manual(values = dest_colors) +
  labs(
...
# Display the plot
print(walking_plot_1986)

# Save the plot if desired
ggsave("walking_plot_1986.jpg", plot = walking_plot_1986, width = 8, height = 6, dpi = 300)


```

## 2015-cycling-impedance
```{r}
library(ggplot2)
library(dplyr)
library(extrafont)
...
# Filter the walking_data for the year 2015 only
cycling_data_2015 <- cycling_data %>%
  filter(YEAR == 2015)
...
# Plot the data for the year 2015
cycling_plot_2015 <- ggplot(cycling_data_2015, aes(x = DURATION, y = f_t, group = dest, color = dest)) +
  geom_line(size = 1.2) +
  scale_color_manual(values = dest_colors) +
  labs(
...
# Display the plot
print(cycling_plot_2015)

# Save the plot if desired
ggsave("cycling_plot_2015.jpg", plot = cycling_plot_2015, width = 8, height = 6, dpi = 300)
...
# Filter the walking_data for the year 2010 only
cycling_data_2010 <- cycling_data %>%
  filter(YEAR == 2010)
...
# Plot the data for the year 2015
cycling_plot_2010 <- ggplot(cycling_data_2010, aes(x = DURATION, y = f_t, group = dest, color = dest)) +
  geom_line(size = 1.2) +
  scale_color_manual(values = dest_colors) +
  labs(
...
# Display the plot
print(cycling_plot_2010)

# Save the plot if desired
ggsave("cycling_plot_2010.jpg", plot = cycling_plot_2010, width = 8, height = 6, dpi = 300)
...
cycling_data_2005 <- cycling_data %>%
  filter(YEAR == 2005)
...
# Plot the data for the year 2015
cycling_plot_2005 <- ggplot(cycling_data_2005, aes(x = DURATION, y = f_t, group = dest, color = dest)) +
  geom_line(size = 1.2) +
  scale_color_manual(values = dest_colors) +
  labs(
...
# Display the plot
print(cycling_plot_2005)

# Save the plot if desired
ggsave("cycling_plot_2005.jpg", plot = cycling_plot_2005, width = 8, height = 6, dpi = 300)
...
cycling_data_1998 <- cycling_data %>%
  filter(YEAR == 1998)
...
# Plot the data for the year 2015
cycling_plot_1998 <- ggplot(cycling_data_1998, aes(x = DURATION, y = f_t, group = dest, color = dest)) +
  geom_line(size = 1.2) +
  scale_color_manual(values = dest_colors) +
  labs(
...
# Display the plot
print(cycling_plot_1998)

# Save the plot if desired
ggsave("cycling_plot_1998.jpg", plot = cycling_plot_1998, width = 8, height = 6, dpi = 300)
...
# Filter the cycling_data for the year 1998 only
cycling_data_1992 <- cycling_data %>%
  filter(YEAR == 1992)
...
# Plot the data for the year 1992
cycling_plot_1992 <- ggplot(cycling_data_1992, aes(x = DURATION, y = f_t, group = dest, color = dest)) +
  geom_line(size = 1.2) +
  scale_color_manual(values = dest_colors) +
  labs(
...
# Display the plot
print(cycling_plot_1992)

# Save the plot if desired
ggsave("cycling_plot_1992.jpg", plot = cycling_plot_1992, width = 8, height = 6, dpi = 300)
...
# Create the plot with specified line colors
ggplot(w_2015_a, aes(x = DURATION, y = f_t, color = dest)) +
  geom_line(size = 1.005) +  # Adjust line thickness
  labs(title = "2015 (a)",
       x = "Time (minutes)",
       y = "f(Cij)",
       color = "Destination") +
  scale_x_continuous(breaks = seq(0, max(w_2015_a$DURATION), by = 10)) +
  scale_color_manual(values = c("home" = "yellow", 
                                "work or school" = "blue", 
                                "other's home" = "green", 
                                "Grocery store, other stores or mall" = "red")) +
  theme(aspect.ratio = 1/2,  # Adjust the aspect ratio if needed
        plot.title = element_text(hjust = 0.5))  # Center-align the title
...
# Create the plot with the modified line style
ggplot(walking_2015_f, aes(x = DURATION, y = f_t, color = dest)) +
  geom_line(size = 1.005) +  # Adjust line thickness and style
  labs(title = "2015 (b)",
       x = "Time (minutes)",
       y = "f(Cij)",
       color = "Destination") +
  scale_x_continuous(breaks = seq(0, max(walking_2015_f$DURATION), by = 10)) +
  theme(aspect.ratio = 1/2, # Adjust the aspect ratio if needed
  plot.title = element_text(hjust = 0.5)) # Center-align the title
...
styled_table_14 <- wide_data %>%
  kable(
    ...
    booktabs = TRUE, 
    longtable = TRUE, 
    caption = "\\label{tab:table_14}Trip Statistics by Mode and Destination for 1986, 1992 and 1998", 
    col.names = c("Destination", "Mode", 
                  "Min 1986", "Median 1986", "Max 1986", "Percentage 1986", 
                  "Min 1992", "Median 1992", "Max 1992", "Percentage 1992",
                  "Min 1998", "Median 1998", "Max 1998", "Percentage 1998"),
    align = 'c'
  ) %>%
