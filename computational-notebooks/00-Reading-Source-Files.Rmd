...
Second, I'll select the required columns (required variables such as RECID (Record identification), EPINO (Sequential episode number), WGHT_EPI (Episode weight), DDAY(Diary - Reference day), TOTEPISO (Total number of episodes during the reference day), ACTCODE (Activity code of the episode), STARTIME (Start time of the episode), ENDTIME (End time of the episode), STARTMIN (Start time of the episode in minutes), ENDMIN (End time of the episode in minutes), DURATION (Duration (in minutes) of the episode) and Place (Location of the episode) and then I'll create origins and destinations columns based on the locations before and after the walking for each respondent.
...
Here, I'll add two columns of year and mode to walking_2010 dataset.
```{r}
walking_2010 <- walking_2010 |>
...
Here a table with origins and destinations for walking in 2010 will be created. First, I'll identify the rows in the episode file where the location of the activity was `17`, which is code for "CYCLING," as well as one row before and after them to identify the locations before and after the cycling that will be considered as the origin and destination for each respondent. 
...
Second, I'll select the required columns (required variables such as RECID (Record identification), EPINO (Sequential episode number), WGHT_EPI (Episode weight), DDAY(Diary - Reference day), TOTEPISO (Total number of episodes during the reference day), ACTCODE (Activity code of the episode), STARTIME (Start time of the episode), ENDTIME (End time of the episode), STARTMIN (Start time of the episode in minutes), ENDMIN (End time of the episode in minutes), DURATION (Duration (in minutes) of the episode) and Place (Location of the episode) and then I'll create origins and destinations columns based on the locations before and after the cycling for each respondent.
...
Here, I'll add two columns of year and mode to cycling_2010 dataset.
```{r}
cycling_2010 <- cycling_2010 |>
  dplyr::select(RECID, WGHT_EPI,ACTCODE, STARTIME:dest_label) |>
  mutate(YEAR = 2010,
...
Here, I'll add two columns of year and mode to walking_2005 dataset.
```{r}
walking_2005 <- walking_2005 |>
  dplyr::select(RECID, WGHT_EPI,ACTCODE, STARTIME:dest_label) |>
  mutate(YEAR = 2005,
...
Here a table with origins and destinations for walking in 2005 will be created. First, I'll identify the rows in the episode file where the location of the activity was `17`, which is code for "CYCLING," as well as one row before and after them to identify the locations before and after the cycling that will be considered as the origin and destination for each respondent. 
...
Here, I'll add two columns of year and mode to cycling_2005 dataset.
```{r}
cycling_2005 <- cycling_2005 |>
  dplyr::select(RECID, WGHT_EPI,ACTCODE, STARTIME:dest_label) |>
  mutate(YEAR = 2005,
...
Here, I'll add two columns of year and mode to walking_1998 dataset.
```{r}
walking_1998 <- walking_1998 |>
  dplyr::select(RECID, WGHTEPI,ACTCODE, STARTIME:dest_label) |>
  mutate(YEAR = 1998,
         MODE = "Walking")
```
...
Second, I'll select the required columns (required variables such as RECID (Record identification), EPINO (Sequential episode number), WGHTEPI (Episode weight), DDAY(Diary - Reference day), TOTEPISO (Total number of episodes during the reference day), ACTCODE (Activity code of the episode), STARTIME (Start time of the episode), ENDTIME (End time of the episode), STARTMIN (Start time of the episode in minutes), ENDMIN (End time of the episode in minutes), DURATION (Duration (in minutes) of the episode) and Place (Location of the episode) and then I'll create origins and destinations columns based on the locations before and after the cycling for each respondent.
...
Here, I'll add two columns of year and mode to cycling_1998 dataset.
```{r}
cycling_1998 <- cycling_1998 |>
  dplyr::select(RECID, WGHTEPI,ACTCODE, STARTIME:dest_label) |>
  mutate(YEAR = 1998,
...
Here, I'll add two columns of year and mode to walking_1992 dataset.
```{r}
walking_1992 <- walking_1992 |>
  dplyr::select(SEQNUM,DDAY, TIMEWGT,ACTCODE, STARTIME:dest_label) |>
  mutate(YEAR = 1992,
         MODE = "Walking")
```
...
walking_1992 |>
  filter(orig_label == "Home" & dest_label == "Home")
```
See RECID == 218 and check the walking episode:
```{r}
gss_e_1992 |>
  filter(SEQNUM == 218) |>
  slice(unique(c(which(PLACE == 7) - 1, 
...
Here, I'll add two columns of year and mode to cycling_1992 dataset.
```{r}
cycling_1992 <- cycling_1992 |>
  dplyr::select(SEQNUM,TIMEWGT,DDAY,ACTCODE:dest_label) |>
  mutate(YEAR = 1992,
         MODE = "cycling")
```
...
Here a table with origins and destinations for walking in 1992 will be created. First, I'll identify the rows in the episode file where the location of the activity was `5`, which is code for "walking," as well as one row before and after them to identify the locations before and after the walking that will be considered as the origin and destination for each respondent. 

```{r creating dataset for walking 1886, include=FALSE, cache=FALSE}

# **walking 1886**
# Creating data of origins and destinations of walking trip 1886
inds = which(gss_e_1986$PLACE == 5)
rows <- lapply(inds, function(x) (x-1):(x+1))
```

Second, I'll select the required columns (required variables such as SEQNUM (Record identification), FWGT_MS (Episode weight), DDAY(Diary - Reference day), ACT_CODE (Activity code of the episode), STARTTIME (Start time of the episode), ENDTIME (End time of the episode), DURATION (Duration (in minutes) of the episode) and Place (Location of the episode) and then I'll create origins and destinations columns based on the locations before and after the walking for each respondent.
...
```{r}
walking_1986 <- walking_1986 |>
  dplyr::select(SEQNUM,DDAY,ACT_CODE:dest_label) |>
  mutate(YEAR = 1986,
         MODE = "Walking")
```
